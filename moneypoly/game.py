...
class _Game:
    """subclass to store some instance attributes"""
    current_index: int
    turn_number: int
    running: bool
...
class Game:
    ...
    def _move_and_resolve(self, player, steps):
        ...
        if tile == "go_to_jail":
            # TODO: should we use GO_TO_JAIL_POSITION ??
            player.go_to_jail()
    ...
    def _handle_jail_turn(self, player):
        ...
        print(f"  {player.name} is in jail (turn {player.jail_state.jail_turns + 1}/3).")

        # Use a Get Out of Jail Free card if available
        if player.jail_state.get_out_of_jail_cards > 0:
        ...
        # No action
        # Serve the turn
        player.jail_state.jail_turns += 1
        if player.jail_state.jail_turns >= 3:
    ...
    def _apply_card(self, player, card):
        ...
        if action == "collect":
            ...
            self._handle_move_card(player, value)
